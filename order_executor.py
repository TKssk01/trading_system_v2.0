...
class OrderExecutor:
    ...
    def execute_orders(self):
        ...
        if abs(price_change) >= price_threshold:
            print("価格変動が閾値を超えました。新規シグナルを発行します。")

            # シグナルを同時に発行
            if price_change > 0:
                # 売り方向の変動
                self.trading_data.set_signal('reverse_sell_repayment', 1)
                print("逆指値の売り返済シグナルを発行しました。")
                # キャンセル処理
                self.cancel_order('reverse_buy_repayment')
                self.reset_signals('reverse_buy_repayment')
            elif price_change < 0:
                # 買い方向の変動
                self.trading_data.set_signal('reverse_buy_repayment', 1)
                print("逆指値の買い返済シグナルを発行しました。")
                # キャンセル処理
                self.cancel_order('reverse_buy_repayment')
                self.reset_signals('reverse_buy_repayment')
            ...
            # 価格変動後のシグナルの評価
            if price_change > 0:
                print("価格が上昇方向に変動しました。売りシグナルを継続します。")
                # 売りシグナルを継続するための追加処理があればここに記述
            elif price_change < 0:
                print("価格が下降方向に変動しました。買いシグナルをキャンセルします。")
                # 買いシグナルをキャンセル
                self.trading_data.set_signal('reverse_buy_repayment', 0)
                print("逆指値の買い返済シグナルをキャンセルしました。")

                # キャンセル処理
                self.cancel_signal('reverse_buy_repayment')
                self.cancel_account_data('reverse_buy_repayment')
